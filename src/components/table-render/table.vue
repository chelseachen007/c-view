...
<template>
  <table>
    <thead>
      <tr>
        <th v-for="(col,index) in columns" :key="`thead${index}`">{{ col.title }}</th>
      </tr>
    </thead>
    <tbody>
      <tr v-for="(row, rowIndex) in data" :key="`tbody${rowIndex}`">
        <td v-for="(col,index) in columns" :key="`tbody_td_${index}`">
